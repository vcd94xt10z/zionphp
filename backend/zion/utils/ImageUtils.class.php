...
class ImageUtils {
     ...
     * @param boolean $lossy
     */
	public static function optimizeUploadFiles(&$files, $lossy=false) {
		for($i = 0;$i < sizeof($files["tmp_name"]);$i++) {
			if(is_array($files["tmp_name"])) {
				$fileSource = $fileDestiny = $files["tmp_name"][$i];
				self::optimizeImage($fileSource, $fileDestiny, $lossy);
				$files["size"][$i] = filesize($files["tmp_name"][$i]);
			}else{
				$fileSource = $fileDestiny = $files["tmp_name"];
				self::optimizeImage($fileSource, $fileDestiny, $lossy);
				$files["size"] = filesize($files["tmp_name"]);
			}
		}
	}
