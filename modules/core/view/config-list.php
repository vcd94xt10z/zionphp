...
<div class="container-fluid">
	...
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">Consulta de Config</li>
		</ol>
	</nav>
	...
	<form class="form-inline hide-advanced-fields ajaxform" action="/zion/rest/core/Config/" method="POST" data-callback="defaultFilterCallback">
		...
		<div class="card">
			...
			<div class="card-body">
				...
				<div class="row row-filter-normal">
					<div class="col-sm-3">
						<label for="filter[env][low]">env</label>
					</div>
					<div class="col-sm-9">
						<select class="form-control filter-operator" id="filter[env][operator]" name="filter[env][operator]">
							<option value=""></option>
							<?foreach(Filter::getOperators() AS $key => $text){?>
							<option value="<?=$key?>"><?=$text?></option>
							<?}?>
						...
						<textarea class="form-control filter-low type-string" id="filter[env][low]" name="filter[env][low]" rows="1"></textarea>
						<textarea class="form-control filter-high type-string" id="filter[env][high]" name="filter[env][high]" rows="1"></textarea>
					</div>
				</div>
				<div class="row row-filter-normal">
					<div class="col-sm-3">
				...
				<div class="row row-filter-advanced">
					<div class="col-sm-3">
						<label for="filter[value][low]">value</label>
					</div>
					<div class="col-sm-9">
						<select class="form-control filter-operator" id="filter[value][operator]" name="filter[value][operator]">
							<option value=""></option>
							<?foreach(Filter::getOperators() AS $key => $text){?>
							<option value="<?=$key?>"><?=$text?></option>
							<?}?>
						...
						<textarea class="form-control filter-low type-string" id="filter[value][low]" name="filter[value][low]" rows="1"></textarea>
						<textarea class="form-control filter-high type-string" id="filter[value][high]" name="filter[value][high]" rows="1"></textarea>
					</div>
				</div>
				<div class="row row-filter-advanced">
					<div class="col-sm-3">
				...
				<div class="row row-filter-advanced">
					<div class="col-sm-3">
						<label for="filter[updated][low]">updated</label>
					</div>
					<div class="col-sm-9">
			...
			<div class="card-footer">
				...
				<a id="button-new" class="btn btn-outline-info" href="/zion/mod/core/Config/new" target="_blank">Novo</a>
			</div>
		</div>
	</form>
